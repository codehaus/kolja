...
public class CommonPages extends AbstractTextReport {
  private StringFrequencies counts = new StringFrequencies();
  ...
  public StringFrequencies getUrlFrequencies() {
    return counts;
  }

  public List<Count> getMostFrequentUrls(int urlCount) {
    return counts.getMostFrequent(urlCount);
  }

  public void display() {
    for (Count c : getMostFrequentUrls(10)) {
      println(c.toString());
    }
  }
}
