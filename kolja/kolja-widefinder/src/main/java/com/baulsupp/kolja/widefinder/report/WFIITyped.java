 ...
 * 
 */
public class WFIITyped extends AbstractTextReport<WFIITyped> {
  ...
  public void processLine(Line l) {
    ...
    if ("GET".equals(line.getMethod())) {
      ...
      HttpStatus status = line.getStatus();
      String client = line.getIpaddress();
      String referrer = line.getReferrer();

      if (isArticle && line.isHitCounted()) {
        articles.increment(url);
        clients.increment(client);

        if (line.isExternalReferrer()) {
